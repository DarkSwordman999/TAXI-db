...
class TaxiApp:
    ...
    def create_widgets(self):
        ...
        self.add_button_group(left, "Администрирование", [
            ...
            ("Создать pg_trgm", self.ensure_pg_trgm),
        ...
        ttk.Label(filters, text="Триграммный поиск:").grid(row=2, column=0, sticky="w", padx=4, pady=3)
        ...
        ttk.Label(filters, text="Порог:").grid(row=2, column=3, sticky="e", padx=4, pady=3)
        ttk.Spinbox(filters, from_=0.05, to=1.0, increment=0.05, textvariable=self.similarity_var, width=6).grid(row=2, column=4, sticky="w", padx=4, pady=3)
        ttk.Button(filters, text="Искать похоже", command=self.quick_search).grid(row=2, column=5, sticky="w", padx=4, pady=3)
        ttk.Button(filters, text="Создать pg_trgm", command=self.ensure_pg_trgm).grid(row=2, column=6, sticky="w", padx=4, pady=3)
    ...
    def get_similarity_threshold(self):
        """Возвращает безопасный порог схожести для pg_trgm."""
        try:
            value = float(str(self.similarity_var.get()).replace(",", "."))
            value = max(0.05, min(1.0, value))
            self.similarity_var.set(str(value))
            return value
        except Exception:
            self.similarity_var.set("0.3")
            return 0.3

    def ensure_pg_trgm(self, silent=False):
        """Создаёт расширение pg_trgm для триграммного поиска."""
        if not self.conn:
            if not silent:
        ...
        try:
            ...
            self.conn.commit()
            if not silent:
                messagebox.showinfo("pg_trgm", "Расширение pg_trgm подключено.")
                self.set_status("pg_trgm подключено")
            return True
        except Exception as e:
            self.conn.rollback()
            if not silent:
                messagebox.showerror("pg_trgm", f"Не удалось подключить pg_trgm:\n{e}")
            return False
    ...
    def quick_search(self):
        """Триграммный поиск по основным текстовым полям базы TAXI."""
        text = self.search_var.get().strip()
        if not text:
            messagebox.showinfo("Поиск", "Введите текст для триграммного поиска")
            return

        if not self.ensure_pg_trgm(silent=True):
            messagebox.showerror(
                "Триграммный поиск",
                "Для триграммного поиска нужно расширение pg_trgm. "
                "Нажмите кнопку 'Создать pg_trgm' или выполните CREATE EXTENSION IF NOT EXISTS pg_trgm."
            )
            return
        ...
        query = """
            WITH q AS (
                SELECT %s::text AS term, %s::text AS like_term, %s::float AS threshold
            )
            SELECT * FROM (
                SELECT
                    'автомобиль' AS тип,
                    a.код::text AS код,
                    a.гос_номер || ' ' || a.марка || ' ' || a.модель || ' ' || COALESCE(a.класс, '') AS найдено,
                    ROUND((GREATEST(
                        similarity(LOWER(COALESCE(a.гос_номер, '')), q.term),
                        similarity(LOWER(COALESCE(a.марка, '')), q.term),
                        similarity(LOWER(COALESCE(a.модель, '')), q.term),
                        similarity(LOWER(COALESCE(a.класс, '')), q.term)
                    ))::numeric, 3) AS схожесть
                FROM АВТОМОБИЛИ a CROSS JOIN q
                WHERE GREATEST(
                        similarity(LOWER(COALESCE(a.гос_номер, '')), q.term),
                        similarity(LOWER(COALESCE(a.марка, '')), q.term),
                        similarity(LOWER(COALESCE(a.модель, '')), q.term),
                   ...
                   OR a.гос_номер ILIKE q.like_term
                   OR a.марка ILIKE q.like_term
                   OR a.модель ILIKE q.like_term
                   OR a.класс ILIKE q.like_term
                ...
                SELECT
                    'водитель' AS тип,
                    v.код::text AS код,
                    v.фамилия || ' ' || v.имя || ' ' || COALESCE(v.телефон, '') AS найдено,
                    ROUND((GREATEST(
                        similarity(LOWER(COALESCE(v.фамилия, '')), q.term),
                        similarity(LOWER(COALESCE(v.имя, '')), q.term),
                        similarity(LOWER(COALESCE(v.телефон, '')), q.term),
                        similarity(LOWER(COALESCE(v.фамилия, '') || ' ' || COALESCE(v.имя, '')), q.term)
                    ))::numeric, 3) AS схожесть
                FROM ВОДИТЕЛИ v CROSS JOIN q
                WHERE GREATEST(
                        similarity(LOWER(COALESCE(v.фамилия, '')), q.term),
                        similarity(LOWER(COALESCE(v.имя, '')), q.term),
                        similarity(LOWER(COALESCE(v.телефон, '')), q.term),
                   ...
                   OR v.фамилия ILIKE q.like_term
                   OR v.имя ILIKE q.like_term
                   OR v.телефон ILIKE q.like_term
                ...
                SELECT
                    'клиент' AS тип,
                    k.код::text AS код,
                    COALESCE(k.фамилия || ' ', '') || k.имя || ' ' || COALESCE(k.телефон, '') AS найдено,
                    ROUND((GREATEST(
                        similarity(LOWER(COALESCE(k.фамилия, '')), q.term),
                        similarity(LOWER(COALESCE(k.имя, '')), q.term),
                        similarity(LOWER(COALESCE(k.телефон, '')), q.term),
                        similarity(LOWER(COALESCE(k.email, '')), q.term),
                        similarity(LOWER(COALESCE(k.фамилия, '') || ' ' || COALESCE(k.имя, '')), q.term)
                    ))::numeric, 3) AS схожесть
                FROM КЛИЕНТЫ k CROSS JOIN q
                WHERE GREATEST(
                        similarity(LOWER(COALESCE(k.фамилия, '')), q.term),
                        similarity(LOWER(COALESCE(k.имя, '')), q.term),
                        similarity(LOWER(COALESCE(k.телефон, '')), q.term),
                   ...
                   OR k.фамилия ILIKE q.like_term
                   OR k.имя ILIKE q.like_term
                   OR k.телефон ILIKE q.like_term
                   OR k.email ILIKE q.like_term
                ...
                SELECT
                    'тариф' AS тип,
                    t.код::text AS код,
                    t.класс || ': ' || t.цена_за_км || ' руб/км, минимум ' || t.минимальная_стоимость AS найдено,
                    ROUND((similarity(LOWER(COALESCE(t.класс, '')), q.term))::numeric, 3) AS схожесть
                FROM ТАРИФЫ t CROSS JOIN q
                WHERE similarity(LOWER(COALESCE(t.класс, '')), q.term) >= q.threshold
                   OR t.класс ILIKE q.like_term
            ) found
            ORDER BY схожесть DESC, тип, найдено
            LIMIT %s
        """
        params = [term, like, threshold, self.get_limit()]
        self.execute_and_display(
            query,
            params,
            title=f"Триграммный поиск: {text} | порог {threshold}",
            status="Триграммный поиск"
        )
